    ...
    while True:
        try:
            ...
            if num_transactions <= 0:
                ...
                workload.append(num_transactions)
                break
        except ValueError:
            print("Please enter a valid integer.")
    ...
    while True:
        try:
            ...
            if scaling_factor <= 0:
                ...
                workload.append(scaling_factor)
                break
        except ValueError:
...
def ddpgTune(workload):
    ...
    for episode_i in range(episode_count):
        ...
        config_reward = env.run_experiment(workload)

        print(f'----------{config_reward}-----------------')

        current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

        with open('./tmpLog/logFile', 'a') as log_file:
