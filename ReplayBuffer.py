...
class ReplayBuffer:
    ...
    def sort_batch(self, batch_size=32):
        ...
        print(f"sort_rewards {sort_rewards}")
        length = len(self.memory)
        print(f"length of memory {length}")
        ...
        # print(f"{self.memory} {current_states} {next_states} {rewards} {actions} {sort_index}")
        # print(f'------------memory------------ {self.memory} ')
        # print(f'current-state {current_states} ')
        # print(f'next-state {next_states} ')
        # print(f'rewards {rewards} ')
        # print(f'actions {actions} ')
        # print(f'sort-index {sort_index} ')    
        # print(np.array(self.memory,dtype="object").shape)
        # print(np.array(sort_index).shape)
        ...
        # print(np.array(self.memory)[sort_index])
        # print(np.array(current_states)[sort_index])
        # print(np.array(next_states)[sort_index])
        # print(np.array(rewards)[sort_index])
        # print(np.array(actions)[sort_index])
        return np.array(self.memory,dtype="object")[sort_index], np.array(current_states)[sort_index], np.array(next_states)[sort_index], np.array(rewards)[sort_index], np.array(actions)[sort_index]
